...
//INSERT TEST CODE START
	...
	if(capListResult.getSuccess()){
		var capList = aa.cap.getCapIDList().getOutput(); //logDebug("---aa.cap.getCapIDList().getOutput() = capList---"); viewObj(capList);
		if(capList.length > 0){
			for(i=0;i<capList.length;i++){
				var thisCapID = capList[i]; logDebug("---capList[i] = thisCapID---"); viewObj(thisCapID);
				
				var capID = thisCapID.getCapID(); logDebug("---thisCapID.getCapID() = capID---"); viewObj(capID);
//				var capId = capID.getCapId(); logDebug("---capID.getCapId() = capId---"); viewObj(capId);
				
				logDebug(capID.getAltID())
				
				//check record opened date
				//check altid for INCOMPLETE
				//get created by
				//get record type
